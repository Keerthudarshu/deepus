<?php
    ...
    // Only show products in the 'girls' category
    $html_product='<div class="product-list list-product-items subpage" page="1">';
    $i=1;
    $j=1;
    $html_iconsubpage='';
    foreach ($_SESSION['product'] as $item) {
      // Try both 'category' and 'gioitinh' fields for compatibility
      $is_girls = false;
      if (isset($item['category']) && strtolower($item['category']) === 'girls') {
        $is_girls = true;
      } elseif (isset($item['gioitinh']) && (strtolower($item['gioitinh']) === 'girls' || $item['gioitinh'] == 2)) {
        // Assuming 'gioitinh' == 2 means girls (check your DB convention)
        $is_girls = true;
      }
      if ($is_girls) {
