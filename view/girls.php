...
<?php
    ...
    <a href="index.php?pg=girls&tatca=1" class="navbar-link">Tất cả</a>
    </li>';
    foreach ($catalog as $item) {
        extract($item);
        $html_catalog.='<li class="navbar-list">
        <a href="index.php?pg=girls&ind='.$id.'" class="navbar-link">'.$name.'</a>
        </li>';
    }

    // Filter products by gioitinh=2 (female)
    $girls_products = array_filter($_SESSION['product'], function($item) {
        return isset($item['gioitinh']) && $item['gioitinh'] == 2;
    });
    ...
    $j=1;
    $html_iconsubpage='';
    foreach ($girls_products as $item) {
